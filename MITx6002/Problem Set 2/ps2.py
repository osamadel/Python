...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        if (int(pos.getX()), int(pos.getY())) not in self.clean_tiles:
            self.clean_tiles.append((int(pos.getX()),int(pos.getY())))
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        if self.room.isPositionInRoom(new_pos):
            # The new position is within the walls
            self.pos = new_pos
            self.room.cleanTileAtPosition(self.pos)
        else:
            self.direction = random.randint(0,360)
...
#            print("A robot is created")
        ...
        while room.getNumCleanedTiles()/room.getNumTiles() < min_coverage:
            ...
            timesteps += 1  # Stores the time-steps needed in this trial.
            for r in robots:
                r.updatePositionAndClean()
        timeSum += timesteps
#        anim.done()
    mean_time = timeSum / num_trials
    return mean_time
                

# Uncomment this line to see how much your simulation takes on average
print(runSimulation(3, 1.0, 20, 20, 0.9, 30, StandardRobot))
...
print(runSimulation(3, 1.0, 20, 20, 0.9, 30, RandomWalkRobot))
